...
- KV namespace/key 管理使用 Cloudflare Go SDK；key 列表通过 `prefix` 参数加载，并在前端按 `/` 推导目录式浏览，支持 root/当前 prefix 面包屑、目录行、当前层 key 批量选择、创建/上传默认落到当前 prefix；KV 诊断快照包含 prefix 和目录/key 列表摘要，但不包含 KV value、binary preview 或下载 URL。
