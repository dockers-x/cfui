...
- OAuth 未配置时，Cloudflare 工作台只突出单个 relay callback 输入框；输入框旁提供复制、保存和检测动作，检测会通过 `/api/oauth/relay-check` 验证 relay `/health` 是否支持 `state-v1`。输入框下方用轻量文字动作提供“使用提供域名”和“自建”，其中“使用提供域名”会填入并保存提供的中转地址，“自建”只打开弹窗显示自建 Worker 的目标回调 URL、自建步骤和脚本；本地 cfui callback 不在未配置引导中突出展示。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
