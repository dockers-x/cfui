...
- D1 数据库创建/删除、详情（file size / table count）回填、SQL 查询控制台、table 浏览、分页行加载、按列类型显示数字/布尔/日期时间/文本输入的行编辑器；可空列能显式写回 SQL NULL，非空值仍通过参数化更新提交，删除按 rowid 参数化执行；前端可复制不含 OAuth/refresh token、SQL 文本、SQL 参数、查询结果行和表格行数据的 D1 诊断快照。
...
| Storage / D1 | Database list, SQL console | Database list/create/delete/detail, SQL console, table browser, type-aware row editor with explicit SQL NULL handling for nullable columns, row update/delete, and no-token/no-row-data diagnostics export | Optional location/jurisdiction fields if exposed through the pinned SDK or a deliberate REST path |
